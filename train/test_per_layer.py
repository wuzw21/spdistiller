        ...
        for layer_idx in range(predictor.num_layers):
            for weight_idx in range(predictor.weight_counters[layer_idx]):
                    with open('test_crosslayer.txt', 'a') as f:
                        f.write(f"Layer {layer_idx}, Weight {weight_idx}:\n")
                        # f.write(str(predictor.activation_counts[layer_idx][weight_idx].tolist()))
                        # sorted_counts, indices = torch.sort(predictor.activation_counts[layer_idx][weight_idx], descending=True)
                        # top_k = int(0.2 * predictor.activation_counts[layer_idx][weight_idx].numel())
                        # top_elements = sorted_counts[top_k]
                        # print(f"Top 20% ratio: {top_elements}")
                        # f.write(f"Top 20% ratio: {top_elements}\n")
                        # f.write(str(sorted_counts.tolist()))
